...
def resample_spectra(data, original_start, original_end, target_len):
    ...
    resampled_data = np.zeros((dimx, dimy, target_len))
...
inpt = read_tiff("./HSI_400-1000nm/WHU-Hi-LongKou.tif") 
...
original_start = 400.0   # e.g., 400 cm-1
original_end = 1200.0 
#original_start = 2641.0 
#original_end = 3067.0   
...
# Ploting two slices of the input SRS image.
...
print(Autoencoder(zdims=32))

# Defining hyper parameters of the model: num of epochs and learning rate
...
learning_rate = 1e-3
num_epochs = 200
model = Autoencoder(zdims=32).cuda()
...
for epoch in range(num_epochs):
  ...
  with torch.no_grad():
    ...
    if loss < best_error:
      ...
      path = F"/media/dataSSD/Python/deepLearning/deepHSI/model/{model_save_name}" 
